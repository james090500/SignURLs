...
public class CustomFunction {
	...
	public static int findKey(String[] line) {
		for (int i = 0; i < line.length; i++) {
			if (line[i].equalsIgnoreCase("[" + Config.SIGN_TEXT + "]") || line[i].equalsIgnoreCase("[" + Config.SIGN_TEXT_COLOR + Config.SIGN_TEXT + ChatColor.BLACK + "]")) {
				return i;
			}
		}
		return -1;
	...
	public static boolean addLink(String lineText, String URL, boolean newLink) {
		if (lineText.length() > 15) {
			...
			if (newLink)
				SignURLs.addLink(lineText, temp);
			list.add(lineText);
			return true;
		}
		return false;
	...
	public static String fetchLink(String lineText) {
		if (linkExists(lineText))
			return link.get(lineText);
		return null;
	}
