...
public class SignLisener implements Listener {
	...
	public void onBlockBreak(BlockBreakEvent event) {
		...
		if (block.getState() instanceof Sign) {
			...
			if (line == -1)
				...
				if (player.hasPermission("signurls.admin")) {
	...
	public void onSignChange(SignChangeEvent event) {
		...
		if (block.getState() instanceof Sign) {
			...
			if (event.getLine(line).equalsIgnoreCase("[" + Config.SIGN_TEXT + "]"))
				event.setLine(line, "[" + Config.SIGN_TEXT_COLOR + Config.SIGN_TEXT + ChatColor.BLACK + "]");

			if (event.getPlayer().hasPermission("signurls.admin")) {
			} else {
			...
			if (line == 3) {
				...
				if (!CustomFunction.linkExists(lineText)) {
					event.getPlayer().sendMessage(SignURLs.PREFIX + "The link " + ChatColor.YELLOW + lineText
							+ ChatColor.WHITE + " could not be found in the DB!");
					block.breakNaturally();
				}
			}
		}
			...
			if (block.getState() instanceof Sign) {
				...
				if (line == -1) {
					...
					if (CustomFunction.linkExists(lineText)) {
						...
						event.getPlayer().sendMessage(SignURLs.PREFIX + "The link '" + ChatColor.YELLOW + lineText+ ChatColor.WHITE + "' could not be found in the DB!");
						block.breakNaturally();
					}
				}
			}
