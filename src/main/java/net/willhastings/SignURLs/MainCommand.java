...
public class MainCommand implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String args[]) {
		...
		if(!sender.hasPermission("signurls.admin")) {
			return false;
		}
		...
		try {
			...
			case ADD: {
				if (args.length < 3) {
					...
					if (args[1].length() > 15) {
						sender.sendMessage(
								SignURLs.PREFIX + " SignText is too long. It can be no longer than 11 charectors!");
					} else {
			...
			case LIST: {
				...
				sender.sendMessage(SignURLs.PREFIX + ChatColor.DARK_GRAY + "|-------------- " + ChatColor.DARK_RED + "Page: " + ChatColor.WHITE + pageNum + ChatColor.DARK_RED + "/" + ChatColor.WHITE + maxPages + ChatColor.DARK_GRAY + " --------------|");
				for (String i : list) {
					if (i == null)
						break;
			...
			case ADMIN: {
				if (args.length < 3) {
					...
					if (CustomFunction.reloadDB())
						sender.sendMessage(SignURLs.PREFIX + "Link DB has been Reloaded!");
					else
						sender.sendMessage(SignURLs.PREFIX + "LinkDB FAILED to Reload!");
				} else if (args[1].equalsIgnoreCase("DATABASE") && args[2].equalsIgnoreCase("PURGE")) {
					if (CustomFunction.purgeDB())
						sender.sendMessage(SignURLs.PREFIX + "Link DB has been purged!");
					else
						sender.sendMessage(SignURLs.PREFIX + "LinkDB FAILED to purge!");
				}
			}
			}
